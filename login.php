...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($errors)) {
        ...
        try {
            ...
            //入力されたメールアドレスを元にユーザ情報を取り出す
            $stmt = $pdo->prepare("SELECT*FROM user WHERE mail =?");
            $stmt->execute(array($input["mail"]));
            $user = $stmt->fetch(PDO::FETCH_ASSOC); //文字列キーにようる配列としてテーブル取得
        ...
        //ユーザ情報が取り出せた　かつ　パスワードが一致すれば、セッションに値を大入試、マイページへ遷移
        if ($user && password_verify($input["password"], $user["password"])) {
            ...
            //「ログイン情報を保持する」にrチェックがあれば、セッションにセットする
